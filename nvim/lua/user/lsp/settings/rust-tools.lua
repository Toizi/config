...
local function on_attach_wrapper(client, bufnr)
    ...
    vim.api.nvim_buf_set_keymap(bufnr, "n", "<leader>lh",
        "<cmd>lua require('rust-tools.inlay_hints').toggle_inlay_hints()<CR>", opts)
    vim.api.nvim_buf_set_keymap(bufnr, "n", "<leader>le",
        "<cmd>lua require('rust-tools.expand_macro').expand_macro()<CR>", opts)
end
...
return {
    server = {
        ...
        settings = {
            -- doesn't seem to make range formatting work
            ['rust-analyzer'] = {
                rustfmt = {
